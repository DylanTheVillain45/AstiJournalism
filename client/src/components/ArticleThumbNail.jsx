  ...
  return (
    <Link to={`/article/${id}`}>
      <div className='px-4 py-4 border-red border-2 bg-beige hover:drop-shadow-2xl transition-all duration-300 mx-1'>
        ...
        <div className='flex items-center gap-8 justify-center'><h3 className='text-slate-600 text-md text-center'>{author}</h3><h3 className='text-slate-600 text-md text-center'><i class="fas fa-thumbs-up text-black opacity-70"></i> {likes}</h3></div>
      </div>
    </Link>
  )
}
